...
import { MIN_ZOOM, MAX_ZOOM, NOTE_TYPES } from '../constants';
...
export default function PlaybackControls({
  ...
  const handleTempoApply = (newTempo, newNoteType) => {
    onTempoChange(newTempo, newNoteType);
  };
  ...
  return (
    <>
      <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 control-panel px-6 py-4 flex items-center gap-5 z-50">
        ...
        {/* Tempo Control - Clickable to open modal */}
        <button
          onClick={() => setIsTempoModalOpen(true)}
          disabled={disabled}
          className={`
            flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-200 glossy-button
            ${disabled
        ...
        >
          <span className="text-xl font-serif">{currentNoteInfo.symbol}</span>
          <span className="text-[#808080] font-medium">=</span>
          <span className="text-lg font-semibold text-[#DAA520] tabular-nums">{tempo}</span>
          <svg className="w-3 h-3 text-[#808080] ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
