  ...
  return (
    <div className="relative w-36 h-36">
      ...
      <svg className="w-full h-full transform -rotate-90 relative z-10" viewBox="0 0 120 120">
        ...
        <defs>
          <linearGradient id="arcGradientModal" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <circle
          ...
          r={radius}
          fill="none"
          stroke={isOvertime ? '#DAA520' : 'url(#arcGradientModal)'}
          strokeWidth="8"
          strokeLinecap="round"
          strokeDasharray={circumference}
          strokeDashoffset={strokeDashoffset}
          filter="url(#glowModal)"
...
export default function UploadModal({
  ...
  onClose,
  onProcessed,
  existingProjectId = null,  // If set, this is a refresh/update operation
  ...
  const processFile = async (file) => {
    ...
    try {
      ...
      // Include the original file for storage
      onProcessed({
        ...data,
        originalFile: file,
        filename: file.name,
        existingProjectId,  // Pass through for updates
        existingProjectName
      });
  ...
  const isRefresh = !!existingProjectId;
  ...
  return (
    ...
    >
      <div className="relative w-full max-w-xl mx-4 p-6 bg-gradient-to-b from-[#1a1a2e] to-[#2C3E50] rounded-2xl border border-[#DAA520]/30 shadow-2xl">
        {/* Close button */}
        {!isProcessing && (
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 rounded-full text-[#808080] hover:text-[#C0C0C0] hover:bg-[#2C3E50] transition-colors"
          >
        ...
        <div className="mb-6 text-center">
          <h2 className="text-xl font-semibold text-[#DAA520]">
            {isRefresh ? `Update: ${existingProjectName}` : 'Upload Sheet Music'}
          </h2>
          <p className="text-sm text-[#808080] mt-1">
            {isRefresh
              ? 'Upload a new version to replace the existing transcription'
              : 'Upload a piano sheet music image for transcription'}
          </p>
        </div>
        ...
        <div
          className={`
            p-8 text-center cursor-pointer rounded-xl border-2 border-dashed
            transition-all duration-200
            ${isProcessing ? 'opacity-70 cursor-not-allowed border-[#DAA520]/20' : ''}
            ${dragActive
              ? 'border-[#DAA520] bg-[#DAA520]/10'
              : 'border-[#DAA520]/30 hover:border-[#DAA520]/60 bg-[#1a1a2e]/50'
            }
          `}
        ...
        >
          ...
          {isProcessing ? (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="space-y-2">
                <p className="text-lg font-medium text-[#C0C0C0]">
        ...
        {error && (
          <div className="mt-4 p-3 bg-[#8B0000]/20 border border-[#C41E3A]/50 rounded-lg text-[#FF6B6B] text-sm">
        ...
        <p className="mt-4 text-center text-xs text-[#606060]">
          Processing typically takes 3-4 minutes. Your project will be saved for instant access later.
        </p>
      </div>
    </div>
  );
